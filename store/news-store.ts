...
const categories = ['All', 'Technology', 'Business', 'Events', 'Education', 'Health'];
  ...
  persist(
    (set, get) => ({
      ...
      fetchArticles: async (category = "", limit = 10, offset = 0) => {
        ...
        try {
          ...
          const res = await fetch(
            `https://social-backend-zid2.onrender.com/news?category=${category.toLowerCase()}&limit=${limit}&offset=${offset}`,
            {
              headers: {
                'token': `${token}`,
      ...
      bookmarkArticle: async(articleId: string) => {
        ...
        try {
          const res = await fetch(`https://social-backend-zid2.onrender.com/news/save/${articleId}`, {
            method: 'POST',
            headers: {
              token: `${token}`
            }
      ...
      likeArticle: async (articleId: string) => {
        ...
        try {
          const res = await fetch(`https://social-backend-zid2.onrender.com/news/like/${articleId}`, {
            method: 'POST',
            headers: {
              token: `${token}`
            }
      ...
      getBookmarkedArticles: async() => {
        try {
          const token = useAuthStore.getState().token;
          const response = await fetch("https://social-backend-zid2.onrender.com/news/saved",{
            headers: {
              token: `${token}`
            }
          })
